...
import { buildGoogleAuthorizationUrl, getGoogleOAuthStateSecret, getGoogleRedirectUri, isGoogleConfigured, requireEnv } from "@/server/google/google-auth";
...
export async function GET(request: Request) {
  ...
  if (!isGoogleConfigured()) {
    return NextResponse.redirect(new URL("/settings?google=unavailable", request.url));
  }
  try {
