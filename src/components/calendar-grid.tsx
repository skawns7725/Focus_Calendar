...
export function CalendarGrid({ blocks }: { blocks: CalendarGridBlock[] }) {
  return (
    <ol aria-label="시간표" className="calendar-grid">
      ...
      {blocks.map((block) => <li key={block.id} data-source={block.source}><span>{format(block.start)} - {format(block.end)}</span><strong>{block.title}</strong><small>{block.source === "quest" ? "퀘스트" : "Google Calendar"}</small></li>)}
    </ol>
  );
}
