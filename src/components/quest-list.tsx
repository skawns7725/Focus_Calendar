...
export function QuestList({ quests, onComplete, onDelete, onNearestDate, onEdit }: { quests: Quest[]; onComplete(id: string): void; onDelete(id: string): void; onNearestDate(id: string): void; onEdit(id: string): void }) {
  ...
  return <section aria-label="우선순위 할 일" className="quest-list">{visible.map((quest) => <QuestCard key={quest.id} quest={quest} onComplete={onComplete} onDelete={onDelete} onNearestDate={onNearestDate} onEdit={onEdit} />)}</section>;
}
