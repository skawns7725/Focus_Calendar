...
export function QuestList({ quests, onComplete, onAbandon }: { quests: Quest[]; onComplete(id: string): void; onAbandon(id: string): void }) {
  ...
  return <section aria-label="우선순위 할 일" className="quest-list">{visible.map((quest) => <QuestCard key={quest.id} quest={quest} onComplete={onComplete} onAbandon={onAbandon} />)}</section>;
}
