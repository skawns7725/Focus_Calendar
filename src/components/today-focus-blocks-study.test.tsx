...
it("shows today's pending study blocks with a study-specific recommendation reason", () => {
  ...
  expect(screen.getByText("지금 실행 가능")).toBeVisible();
...
it("summarizes recommended, completed, remaining, and available focus minutes with the next recommendation", () => {
  ...
  expect(screen.getByText("추천 작업 합계: 75분")).toBeVisible();
  expect(screen.getByText("오늘 가용 시간: 90분")).toBeVisible();
  expect(screen.getByText("완료: 30분")).toBeVisible();
  expect(screen.getByText("남은 계획: 45분")).toBeVisible();
