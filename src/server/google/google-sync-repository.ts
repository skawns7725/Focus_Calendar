...
export class PrismaGoogleSyncRepository {
  ...
  async getCursor(calendarId: string) {
    return (await db.googleCalendarCursor.findUnique({ where: { calendarId } }))?.syncToken ?? null;
  }

  setCursor(calendarId: string, syncToken: string | null) {
    return db.googleCalendarCursor.upsert({
      where: { calendarId },
      create: { calendarId, syncToken },
      update: { syncToken }
    });
  }
  ...
  upsertBlock(block: { externalId: string; calendarId: string; googleEventId: string; title: string; start: string; end: string }) {
    return db.calendarBlock.upsert({
      where: { externalId: block.externalId },
      create: { ...block, start: new Date(block.start), end: new Date(block.end) },
      update: { ...block, start: new Date(block.start), end: new Date(block.end) }
    });
  }

  deleteBlock(calendarId: string, googleEventId: string) {
    return db.calendarBlock.deleteMany({ where: { calendarId, googleEventId } });
  }

  async listQuests(): Promise<Quest[]> {
    return (await db.quest.findMany()).map((quest) => ({
      id: quest.id,
      title: quest.title,
  ...
  listMappings() {
    return db.googleEventMapping.findMany({ select: { questId: true, calendarId: true, googleEventId: true } });
  }

  saveMapping(mapping: { questId: string; calendarId: string; googleEventId: string }) {
    return db.googleEventMapping.upsert({ where: { questId: mapping.questId }, create: mapping, update: mapping });
  }

  deleteMapping(questId: string) {
    return db.googleEventMapping.deleteMany({ where: { questId } });
  }
  ...
  async deleteQuest(id: string) {
    await db.googleEventMapping.deleteMany({ where: { questId: id } });
    return db.quest.deleteMany({ where: { id } });
  }
}
